...
class IfrRecordsController extends AppController
{
	public function index()
	{
		...
		if ($this->request->is(['post'])) {
			...
			if (!empty($district)) {
				$query->where(['IfrRecords.district_id' => $district]);
			}
			if (!empty($taluk)) {
				$query->where(['IfrRecords.taluk_id' => $taluk]);
			}
			if (!empty($village)) {
				$query->where(['IfrRecords.village_id' => $village]);
			}
			$ifrRecords = $query->toArray();
			$taluks            = $this->IfrRecords->find('list', ['keyField' => 'taluk_id', 'valueField' => 'taluk_name'])->where(['IfrRecords.taluk_id' => $taluk, 'IfrRecords.is_active' => 1])->group(['IfrRecords.taluk_name'])->toArray();
			$villages          = $this->IfrRecords->find('list', ['keyField' => 'village_id', 'valueField' => 'village_name'])->where(['IfrRecords.village_id' => $village, 'IfrRecords.is_active' => 1])->group(['IfrRecords.village_name'])->toArray();
		...
		$taluks            = $this->IfrTaluks->find('list', ['keyField' => 'id', 'valueField' => 'name'])->where(['IfrTaluks.district_id' => $district, 'IfrTaluks.is_active' => 1])->group(['IfrTaluks.name'])->toArray();
		$villages            = $this->IfrVillages->find('list', ['keyField' => 'id', 'valueField' => 'name'])->where(['IfrVillages.district_id' => $district, 'IfrVillages.taluk_id' => $taluk, 'IfrVillages.is_active' => 1])->group(['IfrVillages.name'])->toArray();
	...
	public function ajaxcalling($dist = null, $taluk = null, $village = null, $ifr_id = null)
	{
		...
		// print_r($email);
		// exit();
		$ifrRecords = $this->IfrRecords->find('all')->where(['IfrRecords.district_id' => $dist, 'IfrRecords.taluk_id' => $taluk, 'IfrRecords.village_id' => $village])->count();
		// 	echo '<pre>';
		// print_r($users);
		// exit();
	    ...
	    public function ajaxcallingedit($dist = null, $taluk = null, $village = null,$ifr_id = null)
    {
        ...
        // print_r($email);
        // exit();
        $ifrRecords = $this->IfrRecords->find('all')->where(['IfrRecords.district_id' => $dist, 'IfrRecords.taluk_id' => $taluk, 'IfrRecords.village_id' => $village, 'IfrRecords.id !=' => $ifr_id])->count();
        //  echo '<pre>';
        // print_r($users);
        // exit();
