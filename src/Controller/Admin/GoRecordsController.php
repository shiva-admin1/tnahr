 ...
 public function ajaxgorecords()
    {
        ...
        $data = array_map(function ($student) {
            return [
			    'OldrecordId'=>$student->OldrecordId,
	...
	public function add()
	{
		...
		if ($this->request->is(['patch', 'post', 'put'])) {
			...
			$DocumentSubtype       = $this->DocumentSubtypes->find('all')->where(['DocumentSubtypes.id' => $this->request->data['document_subtype_id']])->first();
            $Name = trim(strtoupper($DocumentSubtype['name']));
			...
			if (empty($errors)) {
				if (!empty($this->request->getData('filepath.tmp_name'))) {
					if($this->request->data['year']==""){
						$path='uploads/e-Governance/GoRecords/'.$Name.'/';	
						$savepath='GoRecords/'.$Name.'/';
					}else{
						$path='uploads/e-Governance/GoRecords/'.$Name.'/'.$this->request->data['year'].'/';	
						$savepath='GoRecords/'.$Name.'/'.$this->request->data['year'].'/';						
					}
			...
			if (empty($errors)) {
				if ($i == 0) {					
                   ...
                   if($this->request->data['year']==""){
					...
					$targetPath= $this->request->data['filepath_1'];
				    $newfile= $this->request->data['filename_1'];
				}

				$OldRecords = $this->OldRecords->patchEntity($OldRecords, $this->request->getData());
				$OldRecords->filepath = $savepath;
				$OldRecords->filename = $newfile;
				if ($this->OldRecords->save($OldRecords)) {
