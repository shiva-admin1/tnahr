...
class PreRecordsController extends AppController
{
	public function index()
	{
		...
		if ($this->request->is(['post'])) {
			...
			if (!empty($year_id)) {
				$query->where(['OldRecords.year' => $year_id]);
			}
			if ($document_subtype_id == 135) {
				$condition = ['DocumentSubtypes.is_active' => 1, 'DocumentSubtypes.old' => 1];
				$condition['DocumentSubtypes.district_type'] = 1;
				$condition['DocumentSubtypes.document_type_id'] = $preId;

				$documentsubtype = $this->DocumentSubtypes->find('list', [
					'keyField' => 'id',
					'valueField' => 'name'
				])
				->where($condition)
				->order(['DocumentSubtypes.name' => 'ASC'])
				->toArray();
				if (!empty($document_type)) {
				      $query->where(['OldRecords.document_subtype_id' => $document_type]);
			    }
			}else{
				if (!empty($document_subtype_id)) {
				      $query->where(['OldRecords.document_subtype_id' => $document_subtype_id]);
			    }
			}
			...
			$Years            = $this->OldRecords->find('list', ['keyField' => 'year', 'valueField' => 'year'])->where(['OldRecords.document_type_id' => $preId,'OldRecords.document_subtype_id' => $document_subtype_id,'OldRecords.is_active' => 1])->group(['OldRecords.year'])->toArray();

		}
		$DocumentSubtypes        = $this->DocumentSubtypes->find('list', ['keyField' => 'id', 'valueField' => 'name'])->where(['DocumentSubtypes.is_active' => 1,'DocumentSubtypes.document_type_id' => $preId,'DocumentSubtypes.old' => 1,'DocumentSubtypes.district_type' => 0])->order(['DocumentSubtypes.name ASC'])->toArray();

		$this->set(compact('preRecords', 'DocumentSubtypes', 'Years', 'villages','preId','documentsubtype','DocumentSubtypes','document_type'));
	}
	...
	public function ajaxdocument($id = null)
{
    $this->viewBuilder()->disableAutoLayout(); // for CakePHP 4/5 use this instead of layout('')
    ...
    $taluks = $this->DocumentSubtypes->find('list', [
        'keyField' => 'id',
        'valueField' => 'name'
    ])
    ->where($condition)
