...
class OsrRecordsController extends AppController
{
	...
	public function index()
	{
		...
		if ($this->request->is(['post'])) {
			...
			$district          = $this->request->data['district_id'];
			$taluk             = $this->request->data['taluk_id'];
			$village           = $this->request->data['village_id'];
			$query = $this->OsrRecords->find('all', ['contain' => ['DocumentSubtypes']])
				->where(['OsrRecords.is_active' => 1]);

			if (!empty($district)) {
				$query->where(['OsrRecords.district_id' => $district]);
			}
			if (!empty($taluk)) {
				$query->where(['OsrRecords.taluk_id' => $taluk]);
			}
			if (!empty($village)) {
				$query->where(['OsrRecords.village_id' => $village]);
			}
			...
			$taluks            = $this->OsrRecords->find('list', ['keyField' => 'taluk_id', 'valueField' => 'taluk_name'])->where(['OsrRecords.taluk_id' => $taluk, 'OsrRecords.is_active' => 1])->group(['OsrRecords.taluk_name'])->toArray();
			$villages          = $this->OsrRecords->find('list', ['keyField' => 'village_id', 'valueField' => 'village_name'])->where(['OsrRecords.village_id' => $village, 'OsrRecords.is_active' => 1])->group(['OsrRecords.village_name'])->toArray();
			$this->set(compact('osrRecords', 'districts', 'taluks', 'villages'));
			// echo '<pre>';
			// print_r($osrRecords);
			// exit();
		...
		$taluks            = $this->OsrTaluks->find('list', ['keyField' => 'id', 'valueField' => 'name'])->where(['OsrTaluks.district_id' => $district, 'OsrTaluks.is_active' => 1])->group(['OsrTaluks.name'])->toArray();
		$villages            = $this->OsrVillages->find('list', ['keyField' => 'id', 'valueField' => 'name'])->where(['OsrVillages.district_id' => $district, 'OsrVillages.taluk_id' => $taluk, 'OsrVillages.is_active' => 1])->group(['OsrVillages.name'])->toArray();
		// $villages          = $this->OsrVillages->find('list', ['keyField' => 'village_id', 'valueField' => 'village_name'])->where(['OsrVillages.id' => $village, 'OsrVillages.is_active' => 1])->group(['OsrVillages.name'])->toArray();
...
*/
	...
	public function edit($id = null)
	{
		...
		// $taluks        = $this->OsrTaluks->find('list', ['keyField' => 'id', 'valueField' => 'name'])->where(['OsrTaluks.is_active' => 1])->order(['OsrTaluks.name ASC'])->toArray();
		// $villages        = $this->OsrVillages->find('list', ['keyField' => 'id', 'valueField' => 'name'])->where(['OsrVillages.is_active' => 1])->order(['OsrVillages.name ASC'])->toArray();
		$taluks            = $this->OsrTaluks->find('list', ['keyField' => 'id', 'valueField' => 'name'])->where(['OsrTaluks.district_id' => $osrRecords->district_id, 'OsrTaluks.is_active' => 1])->group(['OsrTaluks.name'])->toArray();
		$villages            = $this->OsrVillages->find('list', ['keyField' => 'id', 'valueField' => 'name'])->where(['OsrVillages.district_id' => $osrRecords->district_id, 'OsrVillages.taluk_id' => $osrRecords->taluk_id, 'OsrVillages.is_active' => 1])->group(['OsrVillages.name'])->toArray();
	...
	public function ajaxtalukoption($val = null)
	{
		...
		$taluks = $this->OsrTaluks->find('all')
			->where(['OsrTaluks.district_id' => $val, 'OsrTaluks.is_active' => 1])
	...
	public function ajaxoptionvillages($district_id = null, $taluk_id = null)
	{
		...
		$villages = $this->OsrVillages->find('all')
			->where(['OsrVillages.district_id' => $district_id, 'OsrVillages.taluk_id' => $taluk_id, 'OsrVillages.is_active' => 1])
			->toArray();
		// echo '<pre>';
		// print_r($villages);
		// exit();
	...
	public function ajaxoptioneditvillages($district_name = null, $taluk_id = null)
	{
		...
		// 	->first();
		$villages = $this->OsrVillages->find('all')
			->where(['OsrVillages.district_id' => $district_name, 'OsrVillages.taluk_id' => $taluk_id, 'OsrVillages.is_active' => 1])
	...
	public function ajaxcalling($dist = null, $taluk = null, $village = null, $osr_id = null)
	{
		// echo '<pre>';
		// print_r($email);
		// exit();
		$osrRecords = $this->OsrRecords->find('all')->where(['OsrRecords.district_id' => $dist, 'OsrRecords.taluk_id' => $taluk, 'OsrRecords.village_id' => $village ])->count();
		// 	echo '<pre>';
		// print_r($users);
		// exit();
		if ($osrRecords > 0) {
	    ...
	    public function ajaxcallingedit($dist = null, $taluk = null, $village = null, $osr_id = null)
    {
        // echo '<pre>';
        // print_r($email);
        // exit();
        $osrRecords = $this->OsrRecords->find('all')->where(['OsrRecords.district_id' => $dist, 'OsrRecords.taluk_id' => $taluk, 'OsrRecords.village_id' => $village, 'OsrRecords.id !=' => $osr_id])->count();
        // echo '<pre>';
        // print_r($osrRecords);
        // exit();
        if ($osrRecords > 0) {
