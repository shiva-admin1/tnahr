...
<div class="row justify-content-center align-items-center bg-red">
    ...
    <div class="col-sm-4 card m-t-1 card-align">
                ...
                <div class="col-md-12" style="padding-right: 5px;">
                    <div class="form-group">
                        ...
                        <input class="md-input form-control" type="email" placeholder="Email Address" name="email"
                            id="email" onblur='uniquefieldvalidation("email",this.value)' ;>
                        <span class="require emailwrng" style="margin-top:20px;display:none;font-size:12px;"><i
                                class="fa fa-at fa-md"></i> Email
                            Already
                            Registered</span>
                ...
                <div class="col-md-6" style="margin-top:4px;">
                    <div class="form-group">
                        ...
                        <span class="green" style="display:none"><i class="fa fa-check-circle"></i>
                            Verified</span><span class="red" style="display:none">Wrong Verification code</span>
                    </div>
                </div>
        ...
        <div class="col-md-6 text-right sign-up">
            <button type="submit" class="btn btn-success">Sign Up &nbsp;<i class="fas fa-sign-out-alt fa-sm"></i></button>
        </div>
        ...
        $.ajax({
            ...
            success: function(data, textStatus) {
                ...
                if (response == 'success') {
                    ...
                    $(".otp_verify").prop("disabled", "disabled").text(
                        "Resend OTP");
                    ...
                    $("#UserMobileNo").parent().removeClass('state-error');
                    $("#UserMobileNo").parent().parent().find("em")
                        .remove();
                    $("#UserMobileOtp").prop("readonly", "readonly");
                    $("#UserMobileNo").parent().removeClass("state-success")
                        .addClass(
                            'state-error');
                    $("#UserMobileNo").parent().parent().append(
                        '<em for="#SchoolId" class="invalid">' +
                        response + '</em>');
                }
            }
        });
...
function uniquefieldvalidation(paramfiled, paramvalue) {
    ...
    $.ajax({
        ...
        dataType: "html",
        type: "get",
        url: "<?php echo $this->Url->build('/', true)?>Users/uniquefieldvalidation/" + paramfiled + "/" +
            paramvalue + "/",
        success: function(data, textStatus) {
            if (data != "Success") {
...
$('#district-id').change(function() {
	 ...
	 if(district_id != ''){
		$.ajax({
			async: true,
			dataType: "html",
			url: "<?php echo $this->Url->build('/', true)?>users/ajaxgettaluk/" + district_id,
			method: 'get',
